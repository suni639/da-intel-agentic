...
* **Primary Source Integrity**: Direct markdown links must use the actual article source URLs (e.g., CoinDesk, BIS) propagated from the ingestion feed, rather than generic search links.
* **Link Normalization**: Malformed, relative, or placeholder links are automatically intercepted and normalized to safe, absolute Google Search query URLs to prevent local 404 errors on the Hugo site or email template.
